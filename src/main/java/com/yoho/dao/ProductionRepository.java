...
public interface ProductionRepository extends JpaRepository<Production	, Long> {
	...
	@Query(value="SELECT TIMESTAMPDIFF (MINUTE, datetime, ?2) as total, isrunning FROM Runtime WHERE machine_name = ?1 ORDER BY id DESC LIMIT 1",nativeQuery = true)
	MachineTotalProjection getLastDurationIsrunning(String machine_name,String dateTo);
